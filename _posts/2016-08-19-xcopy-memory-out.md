...
自从`Windows NT 4.0`开始就成为`windows`资源工具包的一部分，
然后在`Windows Vista`往后就是标准的功能了。
...
```
robocopy Engine\Binaries\Win64 Build\UnrealEngine\Engine\Binaries\Win64 /E //XF *.pdb 
robocopy Engine\Binaries\ThirdParty Build\UnrealEngine\Engine\Binaries\ThirdParty  /E
```